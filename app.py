...
content = {
    "English": {
        ...
        "loading_ai": "AI Generating detailed insight..."
    },
    "தமிழ்": {
        ...
        "loading_ai": "AI விரிவான மருத்துவ அறிக்கையை உருவாக்குகிறது..."
    },
    "हिन्दी": {
        ...
        "loading_ai": "AI विस्तृत नैदानिक रिपोर्ट तैयार कर रहा है..."
    }
}
c = content[lang]
    ...
    with col_out:
        ...
        if analyze:
            try:
                ...
                # Reset AI note when new analysis is triggered
                st.session_state.ai_note = ""
            except:
                st.session_state.risk_score = 10.0 # Fallback
                st.session_state.assessment_done = True
        ...
        if st.session_state.assessment_done:
            ...
            if st.button(c["ai_btn"], use_container_width=True):
                with st.spinner(c["loading_ai"]):
                    sys_prompt = "Professional maternal health assistant. Provide detailed clinical insight."
            ...
            st.info("Please fill in patient vitals and click 'Analyze Health'.")
        st.markdown('</div>', unsafe_allow_html=True)
