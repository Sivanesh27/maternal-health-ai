...
def create_pdf(data_dict, ai_text):
    ...
    if ai_text:
        ...
        # Handle unicode/non-latin characters for PDF compatibility
...
with col_output:
    ...
    if analyze_now or 'risk_score' in st.session_state:
        # Prediction
        ...
        st.metric(label=c["risk_score"], value=f"{risk_score}%")
        
        level = "High Risk" if risk_score > 70 else "Moderate Risk" if risk_score > 40 else "Low Risk"
        if risk_score > 70: st.error(level)
        elif risk_score > 40: st.warning(level)
        ...
        if st.button(c['ai_btn']):
            with st.spinner("AI Generating detailed insight..."):
                prompt = f"Patient: {name}, Age: {age}, Hb: {hb}, BP: {bp}, Weight: {weight}, Week: {week}. Risk: {risk_score}%. Language: {lang}."
        ...
        if 'ai_text' in st.session_state:
            ...
            pdf_bytes = create_pdf(report_data, st.session_state['ai_text'])
            st.download_button(
        ...
        st.info("Input details to generate the assessment.")
    st.markdown("</div>", unsafe_allow_html=True)
