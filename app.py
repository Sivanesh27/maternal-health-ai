...
# --- 2. AI CORE LOGIC (SUPER-DISCOVERY) ---
def call_gemini_ai(prompt, context_type="general", language="English"):
    """
    Super-Discovery Engine: Cycles through every possible model string.
    If all return 404, the API is definitely not enabled in the Google Cloud Console.
    """
    ...
    # Expanded list including Experimental and Latest aliases
    discovery_paths = [
    ...
    full_query = (f"Role: {instr}\n"
                  ...
                  f"USER QUERY: {prompt}\n\n"
                  f"Requirement: Provide a clean, professional response in {language} only.")
    ...
    for version, model_name in discovery_paths:
        ...
        try:
            # surgical 7s timeout per attempt to cycle through all quickly
...
st.write("---")
st.caption(f"MaternalAI Support v3.0 | Exhaustive Discovery Core | Triple Language Sync")
